...
const fn_base: ControllerFunc = async function (request, response): Promise<void> {
    const base_path = path.join(ROOT, FRONTEND_DIR, VIEWS);

    try {
        const base_file = await fs.readFile(path.join(base_path, 'base.html'));
